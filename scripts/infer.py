"""LedgerGuard inference script.

Takes a CSV of transactions and returns scored + explained output as JSON.
...
-----
    python scripts/infer.py \\
        ...
        --out results.json \\
        --tier HIGH         # only surface HIGH-risk transactions (default: all)
"""
...
def run_inference(
    ...
    # Attach original row data to output
    output = []
    for r in results:
        row = df.iloc[r.index].to_dict()
        # Drop NaN / numpy types for JSON serialisation
        clean_row = {
